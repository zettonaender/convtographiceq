...
9. Important: Check the graph produced after this, if the highest frequency (the X-axis) is not 24000 (or whatever you set it to), please change the `length` to fix it. The default setting should work fine, though.
10. Open `run.bat` or with cmd `python periodogram.py`.
...
16. Use the following command to generate graphic eq: `python autoeq.py --input_dir="ssweep" --output_dir="my_results/ssweep" --compensation="compensation/zero.csv" --equalize --show_plot --convolution_eq --fs=48000,44100,192000`
